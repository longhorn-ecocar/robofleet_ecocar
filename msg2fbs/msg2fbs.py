...
def gen_support():
    ...
    # Define a union for payloads.
    yield "union Payload {"
    yield "  // Add payload message types here. For example:"
    yield "  fb.std_msgs.ByteMultiArray,"
    yield "  // AnotherMessage,"
    yield "}"
    
    # Define an enum to discriminate the payload union.
    yield "enum PayloadType : byte {"
    yield "  NONE = 0,"
    yield "  ByteMultiArray = 1,"
    yield "  // Add enum values corresponding to the union members. For example:"
    ...
    # Updated MsgWithMetadata table now includes payload fields.
